...
static void *local_queue_thread_no_crypto(void *arg) {
    ...
    // 3 mảng song song đại diện thông tin của gói tin 
    // Userspace tạo mảng để hứng gói tin từ local đổ gói tin lên
    void *pkt_ptrs[MAX_BATCH_SIZE];         // Địa chỉ dest mac của gói tin để đánh dấu gói tin đầu tiên
